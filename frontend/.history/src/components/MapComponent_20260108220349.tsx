  ...
  const loadRealData = async () => {
    ...
    try {
      ...
      if (locationsResponse.data.success && locationsResponse.data.locations?.length > 0) {
        ...
        // 2. Load weather from Django backend
        console.log('🌤️ Calling getFreeWeather for trip', tripId);
        try {
          const weatherResponse = await api.getFreeWeather(tripId);
          console.log('🌤️ Weather data:', weatherResponse.data);
          
          if (weatherResponse.data && weatherResponse.data.latitude) {
            setWeather(weatherResponse.data);
          }
        } catch (weatherErr) {
          console.warn('🌤️ Weather API error:', weatherErr);
        }
      ...
      <Card className="shadow-2 border-round-xl">
        <div className="flex flex-column justify-content-center align-items-center py-6">
          ...
          <p className="text-sm text-gray-500 mt-2">
            Fetching attractions and weather data
          </p>
        </div>
      </Card>
    );
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {weatherCodes[weather.daily.weathercode[index]] || 'Mixed'}
                          </div>
                        </div>
                      </div>
                    ))}
