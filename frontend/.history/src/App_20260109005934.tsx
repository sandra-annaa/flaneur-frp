...
function TripDashboard() {
 ...
 // In App.tsx - fix the loadUserTrips function
const loadUserTrips = async () => {
  try {
    setIsLoading(true);
    const response = await api.getTrips(); // This returns AxiosResponse
    setUserTrips(response.data); // Use response.data, not the whole response
  } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50">
        <div className="container mx-auto px-4 py-8">
          {/* Header with Logout */}
          <div className="flex justify-content-between align-items-center mb-6 w-full text-center "text-5xl font-bold text-gray-800 font-['Poppins'] tracking-wider absolute left-1/2 transform -translate-x-1/2"">
  <h1 className="text-6xl font-bold text-center text-blue-800 font-['Playfair_Display'] italic tracking-wider">
    Flâneur
  </h1>          <Button
              label="Logout"
              icon="pi pi-sign-out"
...
{userTrips && userTrips.length > 0 && (            <Card className="mb-8">
              <div className="p-4">
