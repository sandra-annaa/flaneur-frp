...
# Add this to accounts/views.py if missing
...
class TripViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        # Automatically assign the current user when creating a trip
        serializer.save(user=self.request.user)

    @action(detail=True, methods=['get'])
    def locations(self, request, pk=None):
        """Custom action to get locations for a trip"""
        return get_free_locations(request, pk)

    @action(detail=True, methods=['get'])
    def weather(self, request, pk=None):
        """Custom action to get weather for a trip"""
        return get_free_weather(request, pk)

    @action(detail=True, methods=['get'])
    def recommendations(self, request, pk=None):
        """Custom action to get recommendations for a trip"""
        return get_recommendations(request, pk)

    @action(detail=True, methods=['post'])
    def generate_itinerary(self, request, pk=None):
        """Custom action to generate itinerary for a trip"""
        return generate_smart_itinerary(request, pk)
        # ========== ADD OTHER MISSING FUNCTIONS ==========

@api_view(['POST'])
@permission_classes([IsAuthenticated])
def debug_create_trip(request):
    """Debug endpoint to create a trip"""
    try:
        data = request.data
        ...
        trip = Trip.objects.create(
            ...
            budget=data.get('budget', 'midrange'),
...
def debug_free_apis(request):
    """Debug endpoint to test free APIs"""
    return Response({
        'status': 'Free APIs configured',
        'services': ['OpenStreetMap', 'Open-Meteo', 'Nominatim'],
        'message': 'All free APIs are properly configured'
    })
